...
while len(correct_states) < len(data_states):
    if answer_state == "Exit":
        missing_states = [state for state in data_states if state not in correct_states]
    ...
    answer_state = screen.textinput(title=f'{len(correct_states)}/50 States Correct', prompt="What's another state's "
                                                                                             "name?").title()
