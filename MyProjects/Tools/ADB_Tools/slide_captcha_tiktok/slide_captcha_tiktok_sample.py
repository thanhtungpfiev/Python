...
class Connect(object):
    ...
    def showDevice(self, serial: str, width: int, height: int, x: int, y: int, title: str):
        ...
        subprocess.Popen(
            f'scrcpy -s {serial} --window-title "{title}" --window-x {x} --window-y {y} --window-width {width} --window-height {height}',
...
class ADB(object):
    ...
    def checkImage(self, serial: str, image: str, *name_luong) -> dict:
        ...
        try:
            ...
            if name_luong:
                subprocess.call(f'adb -s {serial} shell screencap -p /sdcard/screen.png', stdout=subprocess.DEVNULL,
                                stderr=subprocess.STDOUT)
                # time.sleep(2)
                subprocess.call(f'adb -s {serial} pull /sdcard/screen.png {name_luong}/screen.png',
                                stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
                ...
                subprocess.call(f'adb -s {serial} shell screencap -p /sdcard/screen.png', stdout=subprocess.DEVNULL,
                                stderr=subprocess.STDOUT)
                # time.sleep(2)
                subprocess.call(f'adb -s {serial} pull /sdcard/screen.png screen.png', stdout=subprocess.DEVNULL,
                                stderr=subprocess.STDOUT)
    ...
    def openApp(self, serial: str, packagename: str):
        ...
        """
        subprocess.call(f'adb -s {serial} shell "monkey -p {packagename} -c android.intent.category.LAUNCHER 1"',
                        stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
    ...
    def clearData(self, serial: str, packagename: str):
        ...
        """
        subprocess.call(f'adb -s {serial} shell "pm clear {packagename}"', stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT)
    ...
    def checkInstallApk(self, serial: str, packagename: str):
        ...
        """
        try:
            apps = subprocess.check_output('adb -s %s shell pm list packages' % (serial)).decode('utf8').strip().split(
                '\r\r\r\n')
            app_check = False
            for i in apps:
                if packagename in i:
    ...
    def changeProxy(self, serial: str, ip: int, port: int):
        ...
        """
        subprocess.call(f'adb -s {serial} shell settings put global http_proxy {ip}:{port}', stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT)
    ...
    def remProxy(self, serial: str):
        ...
        Thêm Proxy Http IP:PORT
        """
        subprocess.call(f'adb -s {serial} shell settings put global http_proxy :0', stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT)
    ...
    def inputSwipe(self, serial: str, x: int, y: int, finishX: int, finishY: int, *dur: int):
        if dur:
            subprocess.call(f'adb -s {serial} shell input swipe {x} {y} {finishX} {finishY} {dur[0]}',
                            stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
        else:
            subprocess.call(f'adb -s {serial} shell input swipe {x} {y} {finishX} {finishY}', stdout=subprocess.DEVNULL,
                            stderr=subprocess.STDOUT)

    def inputText(self, serial: str, text: str):
        charsb64 = str(base64.b64encode(text.encode('utf-8')))[1:]
        subprocess.call('adb -s %s shell am broadcast -a ADB_INPUT_B64 --es msg %s ' % (serial, charsb64),
                        stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
        # subprocess.call('adb -s %s "shell input text \\"%s\\"" '%(serial,text))

    def inputKeyEvent(self, serial: str, key: int):
        subprocess.call(f'adb -s {serial} shell input keyevent {key}', stdout=subprocess.DEVNULL,
                        stderr=subprocess.STDOUT)
    ...
    def startLink(self, sr: str, action: str, packagename: str):
        subprocess.call(f"adb -s {sr} shell am start -a android.intent.action.VIEW -d {action} {packagename}",
                        stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
    ...
    def dumpXml(self, serial: str):
        ...
        subprocess.call(f'adb -s {srdev} pull /sdcard/uidump.xml "{os.getcwd()}\\{serial}\\ui.xml"',
                        stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
...
def slideCaptcha(sr, adb):
    ...
    if checktext:
        ...
        if check and check['status'] == "success":
            ...
            adb.inputSwipe(sr, round(check['x']), round(check['y']), int(check['x']) + int(captcha), round(check['y']),
                           1000)
        return True
    else:
        return False
